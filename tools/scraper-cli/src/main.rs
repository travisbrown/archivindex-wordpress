...
use archivindex_archiver::capture::{CaptureControl, CaptureEvent, CaptureSummary};
...
use archivindex_cli_support::{CommandOutcome, Verbosity, exit_code, interrupt_flag};
...
fn run_archive_for_session(
    ...
    let mut session = Session::new(archiver, &session_name, &mut state, &output)?.events(
...
/// Own traversal, recorded progress, and its display for one archive run.
/// The checkpoint is usable only after the session successfully publishes its archive.
...
enum Error {
    ...
    #[error(transparent)]
    ConfigFile(#[from] archivindex_cli_support::config::ConfigError),
    #[error(transparent)]
...
mod tests {
    ...
    use archivindex_cli_support::CommandOutcome;
    use archivindex_cli_support::config::load_config;
    ...
    use super::{
        ...
        page_total_change_warning, parse_per_page, resume_archive, resume_command, resume_info,
        run_archive, session_name_from_warc,
    };
