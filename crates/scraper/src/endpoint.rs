...
impl EndpointType {
    ...
    ///
    /// The response is a JSON object keyed by slug. Its key order is the order custom endpoints are
    /// archived in, so the entries are read as a sequence rather than a map.
    ///
    /// # Errors
    ///
