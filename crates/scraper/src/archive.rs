...
//! resources, discovers custom collections from the type and taxonomy registries among them, probes
//! every supported [`Endpoint`] and then every custom collection with a bare request, and finally
//! pages each exposed collection in that order. Its [`Checkpoint`] records where a stopped run can
//! resume.
...
impl Site {
    /// Parse a host with an optional installation path, such as `example.com/blog`.
    ///
    /// An `http://` or `https://` scheme is optional; HTTPS is the default. Trailing slashes are
    /// removed, and only an explicit `http://` scheme is retained in [`base`](Self::base).
    ///
    /// # Errors
    ///
    /// Returns [`SiteError`] if the base cannot be parsed or contains a query or fragment.
    ...
    /// A session name made from the scheme-free base and the Unix timestamp at `at`.
    ///
    /// Characters other than ASCII letters, digits, `-`, `.`, `_`, and `~` are replaced with
    /// hyphens before the timestamp is appended.
    #[must_use]
...
/// The point from which a stopped run can resume.
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum Checkpoint {
    ...
    Initial,
    /// Resume a collection after its last captured page.
    Resume(Resumption),
    /// Every exposed collection finished pagination.
    Finished,
}

/// The collection and last captured page from which to resume.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct Resumption {
    /// The endpoint to continue.
    pub endpoint: Collection,
    /// The last captured page, or zero when no paginated request has completed.
    pub last_page: usize,
    /// The greatest page count observed during pagination, if available.
    pub total_pages: Option<usize>,
}

/// Progress through one exposed collection whose probe or resume checkpoint supplied a page count.
#[derive(Clone, Debug, Eq, PartialEq)]
...
///
/// Captures the API roots and probes every supported [`Endpoint`], then probes custom collections
/// discovered in the type and taxonomy registries, in registry order (types before taxonomies).
/// Custom probes link back to their registry.
///
/// After probing, collections with a 2xx or 304 probe response are paged in probe order. Each
/// collection gets one pass, from page one through the greatest `X-WP-TotalPages` value observed
/// during pagination. Every page carries the `before` cutoff and links to the preceding page, or to
/// the probe for page one. A later page returning the recognized invalid-page error ends the pass.
/// Other probe responses skip the collection, except that a Cloudflare challenge stops the session.
///
/// Unexpected page responses, unreadable registries, and capture failures stop the run. Use
/// [`checkpoint`](Self::checkpoint) to find its continuation point and [`resume`](Self::resume) or
/// [`resume_with_probes`](Self::resume_with_probes) to construct a resumed driver.
...
impl ArchiveDriver {
    ...
    /// The checkpoint to use if the run stops now.
    #[must_use]
    ...
    /// The supported endpoints followed by the custom collections discovered so far, in the order
    /// they are probed.
    #[must_use]
    ...
    fn inspect_probe(&mut self, endpoint: Collection, capture: &Capture<'_>) {
        ...
        // progress from its item total at the configured page size instead; only a count carried
