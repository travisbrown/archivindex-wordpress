...
pub enum Error {
    ...
    /// A generated capture record has no complete physical frame.
...
/// for a missing page after page one names the preceding page URL as its `via`. Output compression
/// is selected from the output filename: names ending in `.gz` are gzip-compressed and all other
/// names are plain WARC files. When captures are in progress, each completed capture is flushed to
/// `<output>.partial`. When no pages are missing, the output contains only the original `warcinfo`
/// record.
...
mod tests {
    ...
    fn completion_compression_follows_the_output_filename() -> Result<(), Box<dyn std::error::Error>>
    {
        ...
        for (name, expected_gzip) in [("output.warc", false), ("output.warc.gz", true)] {
            ...
            CompletionOutput::new(&output, &source)?.publish()?;
    ...
    #[test]
    fn completion_drops_partial_output_and_refuses_a_competing_destination()
    -> Result<(), Box<dyn std::error::Error>> {
        let directory = tempfile::tempdir()?;
        let output = directory.path().join("output.warc.gz");
        let source: Record = Record::warcinfo(Utc::now()).build();
        let source = source.into_raw()?;
        drop(CompletionOutput::new(&output, &source)?);
        assert!(!output.exists());
        assert!(!partial_path(&output).exists());
        let completion = CompletionOutput::new(&output, &source)?;
        std::fs::write(&output, b"other writer")?;
        assert!(
            matches!(completion.publish(), Err(super::Error::Publication(error)) if !error.is_published())
        );
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(requests, [("GET".to_owned(), expected_target.to_owned())]);
