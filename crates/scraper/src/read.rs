//! Reading `WordPress` REST API v2 comments from WARC files.
...
pub enum Error {
    ...
    /// A comments response does not contain a valid HTTP message.
...
/// ID. Every pair of distinct versions sharing an ID is included in the warnings. IDs are not
/// scoped by site, so use this function with a single site's comments. Revisit records are ignored.
...
///
/// For each site, the greatest valid `date_gmt` among its archived comments is preferred. Only when
/// that site has no comment datetime is the greatest valid `before` value on one of its archived
/// comments response or revisit URLs used. Anchors are returned in domain-name order.
...
/// `WARC-Identified-Payload-Type` set to `application/json`. The greatest valid `X-WP-TotalPages`
/// value on a qualifying record determines the required range. Every transition in that value is
/// retained so callers can warn about pagination changes. Duplicate captures satisfy the page only
/// once. An archive without a valid advertised total is incomplete. This single-collection
/// convenience function rejects a WARC containing several endpoints; use
...
///
/// Collections are returned in domain-name order, with coverage and advertised totals tracked
/// separately for each endpoint. An archive without a qualifying capture returns an empty vector.
