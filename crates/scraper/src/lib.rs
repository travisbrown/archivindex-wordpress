...
//! # Modules
//!
//! * [`archive`]: archiving supported and registry-advertised collections
...
use archivindex_wordpress_model::api::resource::Comment;
...
///
/// The driver takes a window of comment creation times when it is constructed. Its first request is
/// a seed for page one of the comments in ascending ID order within that window, also given by
...
/// and the number of visible comments does not exceed `X-WP-Total`. `WordPress` applies per-comment
/// read checks after its pagination query, so the reported total can legitimately exceed the number
/// of comments returned. A second sweep runs when the consistency checks fail, or when explicitly
/// requested with [`second_sweep`](Self::second_sweep). The fixed cutoff prevents ordinary
/// additions dated after the cutoff from entering the result set. Captured IDs are retained even
/// if they are deleted during collection.
///
...
/// page the server answers with `304 Not Modified` adds no IDs, and the sweep continues by the page
/// count last advertised. Malformed JSON or an unexpected HTTP response makes the session
...
impl CommentDriver {
    /// Create a comment driver restricted to a fixed creation-time window.
    ///
...
/// too, so the driver stops the traversal rather than failing page by page.
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(driver.next(), Some(page_request(&driver, 2, None)));
        // ID 1 is deleted before page 2 is requested, reducing the collection to one page, so the
        // validation sweep begins via the page that vanished.
