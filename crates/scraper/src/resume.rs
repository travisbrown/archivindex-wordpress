...
//!
//! [`inspect_archive`] links each request, response or revisit, and metadata record into a capture,
//! then replays complete captures through [`ArchiveDriver`]. This uses the same endpoint discovery
//! and checkpoint transitions as a live archive. A broken capture is reported but does not prevent
//! the last durable checkpoint from being returned.
...
///
/// A request, its linked response or revisit, and the response's linked metadata record must all be
/// present for the capture to advance the checkpoint. Missing, orphaned, duplicated, truncated, or
/// unreadable capture records are returned as warnings. Record parsing errors remain fatal because
/// the next record boundary cannot be trusted after one.
///
/// # Errors
///
...
/// collections, including for continuation files produced without redundant bare probes. `before`
/// supplies the initial segment's cutoff when this segment contains no paginated URL from which to
/// recover it.
///
/// # Errors
///
