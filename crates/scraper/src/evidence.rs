...
//!
//! Requests open groups in source order. Responses link to requests already seen, metadata links to
//! an accepted response already seen, and revisits resolve only earlier responses. Forward
//! references remain unlinked; this is the ordering supported by both consumers. Duplicate request
//! IDs select the latest request; the first response of a group wins. Metadata occurrences are
//! retained in order so consumers can choose their own policy.
...
mod tests {
    ...
    fn resume_and_lint_report_the_same_relationship_problem() {
        ...
        // Resume uses the last typed metadata via (ignoring a later raw block), while lint uses the
        // first metadata for the root's no-via requirement.
        assert!(
            resume
                .endpoints
