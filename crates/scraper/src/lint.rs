//! Structural validation of collection archives produced by
//! [`ArchiveDriver`](crate::archive::ArchiveDriver).
...
pub struct LintReport {
    /// The findings of the pass: those of the standard rules and those of the rules below, in the
    /// order the pass reported them.
...
///
/// One pass checks the file against the rules of [`archivindex_warc_ops::lint`] and the `WordPress`
/// rules below. The archive must begin with all API roots and known probes, followed by
/// registry-advertised custom probes. Every successful probe must have one correctly linked
/// pagination traversal. Legacy second passes are also checked when the collection spans more
/// than one page.
///
/// # Errors
///
/// Returns [`Error`] when the file cannot be read as a WARC or no `WordPress` request identifies
/// the installation. Protocol violations are returned as findings rather than errors.
...
///
/// Findings are reported after the file has been read, when all capture relationships are known.
...
///
/// Collects capture relationships as records are read and checks them when the file ends.
/// Unreadable records are excluded from these checks; their missing relationships can produce
/// incomplete-capture findings.
...
impl WordPressRules {
    /// Create a rule set, using `path` to identify the archive in errors.
    #[must_use]
